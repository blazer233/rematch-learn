...
const __spreadArrays = (...arg) => arg.flat();
...
var validate = function (validations) {
  {
    for (
      var _i = 0, validations_1 = validations;
      _i < validations_1.length;
      _i++
    ) {
 ...
 */
var isListener = function (reducer) {
...
function createRedux(_a) {
  ...
  var enhancers = composeEnhancersWithDevtools(redux.devtoolOptions).apply(
    void 0,
    __spreadArrays(redux.enhancers, [middlewares])
  );
...
var Rematch = /** @class */ (function () {
  ...
  Rematch.prototype.getModels = function (models) {
    return Object.keys(models).map(function (name) {
      return __assign(__assign({ name: name }, models[name]), {
  ...
  Rematch.prototype.init = function () {
    ...
    var rematchStore = __assign(
      __assign({ name: this.config.name }, redux.store),
      {
...
var mergeConfig = function (initConfig) {
  var config = __assign(
    __assign({ name: initConfig.name, models: {}, plugins: [] }, initConfig),
    {
      redux: __assign(
        __assign(
  ...
  for (var _i = 0, _a = config.plugins; _i < _a.length; _i++) {
    ...
    if (plugin.config) {
      ...
      if (plugin.config.redux) {
        ...
        config.redux.enhancers = __spreadArrays(
          config.redux.enhancers,
          plugin.config.redux.enhancers || []
        );
        config.redux.middlewares = __spreadArrays(
